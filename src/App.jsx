...
import {
  ...
  CheckCircle2, Check, MonitorPlay, BookA, ChevronDown, FileText, Mic, ExternalLink
} from 'lucide-react';
...
const App = () => {
  ...
  // ★ 웹 교재를 담을 그릇 (여기에 스위치를 강제 연결할 겁니다)
  const webContentRef = useRef(null);

  // ==========================================
  // [단어장 상태 및 로직 (기존과 완벽 동일)] 
  // ==========================================
  ...
  // ★ [핵심] 리액트가 무시한 버튼 기능을 강제로 연결하는 마법의 코드 ★
  useEffect(() => {
    if (appMode !== 'class' || contentTab !== 'pdf' || !webContentRef.current || !selectedLesson?.web_content) return;

    const container = webContentRef.current;

    // 1. 플립 카드 (카드 뒤집기) 기능 연결
    const flipCards = container.querySelectorAll('.flip-card');
    flipCards.forEach(card => {
      card.onclick = function() { this.classList.toggle('flipped'); };
    });

    // 2. 토글 스위치 (Me / Others) 기능 연결
    const switches = container.querySelectorAll('.tk-switch input[type="checkbox"]');
    switches.forEach(sw => {
      sw.onchange = function() {
        const id = this.id.replace('switch-', '');
        const korText = document.getElementById('kor-' + id);
        const engText = document.getElementById('eng-' + id);
        if(korText && engText) {
          korText.innerText = this.checked ? korText.getAttribute('data-opt2') : korText.getAttribute('data-opt1');
          engText.innerText = this.checked ? engText.getAttribute('data-eng2') : engText.getAttribute('data-eng1');
          korText.style.color = this.checked ? '#526ae5' : '#1e293b';
        }
      };
    });
    ...
    quizOptions.forEach(opt => {
      opt.onclick = function() {
        ...
        const isCorrect = onclickAttr.includes('true'); // onclick 문자열에 true가 있으면 정답
        const feedbackMatch = onclickAttr.match(/'([^']+)'/);
        const feedbackId = feedbackMatch ? feedbackMatch[1] : 'feedback-nuance';

        const parent = this.closest('.flex-col');
        if(parent) {
        ...
        if (isCorrect) {
          this.style.borderColor = "#22c55e"; this.style.backgroundColor = "#f0fdf4"; this.style.color = "#15803d";
          if(resultText) { resultText.innerText = "✅ Correct!"; resultText.style.color = "#22c55e"; }
        } else {
          this.style.borderColor = "#ef4444"; this.style.backgroundColor = "#fef2f2"; this.style.color = "#b91c1c";
                    ...
                    {contentTab === 'pdf' ? (
                      selectedLesson.course === 'MAIN233' ? (
                        selectedLesson.pdf_url ? (
                          <div className="bg-[#525659] p-2 md:p-4 flex flex-col items-center custom-scrollbar h-[600px] md:h-[800px] overflow-y-auto rounded-b-[2rem]">
                            <Document 
                              ...
                              error={<div className="text-white bg-red-500/20 p-4 rounded-xl text-center text-sm">PDF를 불러오지 못했습니다.<br/>(크롬 확장프로그램이나 백신이 차단했을 수 있습니다.)</div>}
                            >
                        ...
                        selectedLesson.web_content ? (
                          /* ★ 서브강의 HTML 그릇 (여기에 useRef를 연결했습니다!) ★ */
                  ...
                  <main className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
                    <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                      <div className="lg:col-span-5 space-y-6">
                        ...
                        <div className="bg-purple-600 rounded-3xl p-8 md:p-10 text-white shadow-xl shadow-purple-600/20 min-h-[300px] flex flex-col justify-center relative overflow-hidden group"><div className="relative z-10"><span className="text-white/50 text-[10px] font-bold uppercase tracking-widest block mb-4">Pattern {currentExIdx + 1}: {activeWord.examples[currentExIdx]?.type}</span><h3 className="text-3xl md:text-4xl font-bold mb-4 korean-text break-keep leading-snug">{activeWord.examples[currentExIdx]?.ko}</h3><p className="text-white/70 text-lg mb-10 font-medium italic">{activeWord.examples[currentExIdx]?.en}</p><button onClick={() => playAudio(getAudioUrl(activeWord.id, currentExIdx), 'example', `${activeWord.id}_${currentExIdx}`)} className="w-16 h-16 bg-white text-purple-600 rounded-full flex items-center justify-center shadow-lg hover:scale-110 active:scale-95 transition-transform"><Volume2 size={32} className="fill-current" /></button></div><div className="absolute top-0 right-0 w-48 h-48 bg-white/10 rounded-full -mr-24 -mt-24 blur-3xl"></div></div>
                      </div>
